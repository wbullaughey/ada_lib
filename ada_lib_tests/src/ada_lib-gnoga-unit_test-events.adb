...
-- procedure Mouse_Move_Handler is new Mouse_Move.Move_Handler ("move");
   ...
   ---------------------------------------------------------------
      ...
      Down_Key_Event    : constant Standard.Gnoga.Gui.Base.
                              Keyboard_Event_Record := (
                                    Message     => Standard.Gnoga.Gui.Base.Key_Down,
                                    Key_Code    => 100,
                           Key_Char    => Ada.Characters.Handling.
                                             To_Wide_Character ('B'),
                                    Alt         => False,
                                    Control     => False,
                                    Shift       => False,
      ...
      Press_Key_Event   : constant Standard.Gnoga.Gui.Base.
                              Keyboard_Event_Record := (
      ...
      Up_Key_Event      : constant Standard.Gnoga.Gui.Base.
                              Keyboard_Event_Record := (
   ...
   begin
      Log_In (Debug, (if Connection_Data = Null then "null data" else
         "data address " & Image (Connection_Data'address) &
         (if Connection_Data.Main_Window = Null then " null main window" else
            "have main window")));
      Connection_Data.Main_Window.On_Character_Handler (
         Character_Event_Handler'access);
      Connection_Data.Main_Window.On_Key_Down_Handler (
         Keyboard_Event_Handler'access);
      Connection_Data.Main_Window.On_Key_Up_Handler (
         Keyboard_Event_Handler'access);
      Connection_Data.Main_Window.On_Key_Press_Handler (
         Keyboard_Event_Handler'access);
      ...
      if not Options.Manual then
         Assert (Connection_Data.Down_Key = Down_Key_Event,
            "did not get expected Down_Key " &
            Ada.Characters.Handling.To_Character (Down_Key_Event.Key_Char) &
            " got '" & Ada.Characters.Handling.To_Character (
               Connection_Data.Down_Key.Key_Char) & "'");
         Assert (Connection_Data.Key = Key, "did not get expected Key " & Key &
            " got '" & Connection_Data.Key & "'");
         Assert (Connection_Data.Press_Key = Press_Key_Event,
            "did not get expected Press_Key " &
            Ada.Characters.Handling.To_Character (Press_Key_Event.Key_Char) &
            " got '" & Ada.Characters.Handling.To_Character (
               Connection_Data.Press_Key.Key_Char) & "'");
         Assert (Connection_Data.Up_Key = Up_Key_Event,
            "did not get expected Up_Key " &
            Ada.Characters.Handling.To_Character (Up_Key_Event.Key_Char) &
            " got '" & Ada.Characters.Handling.To_Character (
               Connection_Data.Up_Key.Key_Char) & "'");
      end if;
   ...
   begin
      ...
      Event_Connection_Data_Access (
         Ada_Lib.GNOGA.Get_Connection_Data).Got_Click := False; -- clear for next event
      ...
      if Options.Manual then
         ...
         Connection_Data.Main_Window.Fire_On_Mouse_Click (
            Auto_Mouse_Event_1);
      end if;
      Log_Out (Debug);
   end Mouse_Click;
...
-- end Mouse_Drag;
   ...
   begin
      ...
      Ada_Lib.GNOGA.Set_Connection_Data (new Event_Connection_Data_Type);
